...
class PostRepository(BaseRepository[Post, PostCreate, PostUpdate]):
    ...
    async def get_published_posts(self, db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Post]:
        query = (
            select(Post)
            .where(and_(Post.status == PostStatus.published, not Post.is_deleted))
            .offset(skip)
            .limit(limit)
    ...
    async def get_user_posts(self, db: AsyncSession, user_id: int, skip: int = 0, limit: int = 100) -> List[Post]:
        query = (
            select(Post)
    ...
    async def get_active_post_by_id(self, db: AsyncSession, post_id: int) -> Post | None:
        query = select(Post).where(and_(Post.id == post_id, not Post.is_deleted))
        result = await db.execute(query)
...
class CommentRepository(BaseRepository[Comment, CommentCreate, CommentUpdate]):
    ...
    async def get_by_post(self, db: AsyncSession, post_id: int, skip: int = 0, limit: int = 100) -> List[Comment]:
        query = (
            select(Comment)
...
class LikeRepository(BaseRepository[Like, Any, Any]):
    ...
    def create_like(self, db: AsyncSession, post_id: int, user_id: int) -> Like:
